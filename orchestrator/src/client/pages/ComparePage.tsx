...
const CompareQuickActions: React.FC<{
	...
	onApplied: () => void;
}> = ({ section, otherProfileUrl, disabled, designResumeExists, onApplied }) => {
	const [applying, setApplying] = useState<"copy" | "copy_rewrite" | null>(null);
...
function renderOwnSectionContent(
	...
	if (section === "basics") {
		...
		if (!b) return <p className="text-muted-foreground text-sm italic">No profile data</p>;
		return (
			<div className="space-y-1 text-sm">
				...
				{b.summary && <p className="text-muted-foreground line-clamp-3">{b.summary}</p>}
				{b.location?.city && (
	...
	if (section === "experience" && typeof sectionData === "object" && "items" in sectionData) {
		...
		if (items.length === 0) return <p className="text-muted-foreground text-sm italic">No items</p>;
		return (
					...
					return (
						<div key={String(it.id ?? i)} className="text-sm">
							...
							<p className="text-xs text-muted-foreground">
								{String(it.company ?? "")} {it.date ? `· ${String(it.date)}` : ""}
							</p>
						</div>
					);
				})}
	...
	if (typeof sectionData === "object" && "content" in sectionData) {
		const content = String((sectionData as { content?: unknown }).content ?? "");
		return content ? (
		...
		return (
			<div className="flex flex-wrap gap-1">
				{items.slice(0, 8).map((item: unknown, i) => {
					const it = item as Record<string, unknown>;
					return (
						<Badge key={String(it.id ?? i)} variant="secondary" className="text-xs">
...
function renderOtherSectionContent(
	...
	if (section === "basics") {
		return (
			<div className="space-y-1 text-sm">
				{profile.basics.name && <p className="font-medium">{profile.basics.name}</p>}
				{profile.basics.headline && (
					<p className="text-muted-foreground">{profile.basics.headline}</p>
				)}
				{profile.basics.summary && (
					<p className="text-muted-foreground line-clamp-3">{profile.basics.summary}</p>
				)}
				{profile.basics.location && (
					<p className="text-muted-foreground text-xs">{profile.basics.location}</p>
				)}
			</div>
		);
	}
	...
	if (section === "experience") {
		return (
			<div className="space-y-2">
				{(items as NormalisedCompareProfile["sections"]["experience"]).slice(0, 3).map((item, i) => (
	...
	if (section === "skills") {
		return (
			<div className="flex flex-wrap gap-1">
				{(items as NormalisedCompareProfile["sections"]["skills"]).slice(0, 8).map((item, i) => (
	...
	if (section === "education") {
		return (
			<div className="space-y-2">
				{(items as NormalisedCompareProfile["sections"]["education"]).slice(0, 3).map((item, i) => (
					<div key={i} className="text-sm">
						<p className="font-medium">{item.school}</p>
						<p className="text-xs text-muted-foreground">
							{[item.degree, item.area].filter(Boolean).join(", ")} {item.period ? `· ${item.period}` : ""}
						</p>
					</div>
				))}
				{items.length > 3 && (
	...
	return (
		<div className="flex flex-wrap gap-1">
			{items.slice(0, 6).map((item: Record<string, unknown>, i) => (
				<Badge key={i} variant="secondary" className="text-xs">
...
const CompareSectionRow: React.FC<{
	...
	evaluating: boolean;
	otherProfileUrl: string;
	applyingSection: CompareSectionKey | null;
	designResumeExists: boolean;
	onApplied: () => void;
}> = ({
	section,
...
export const ComparePage: React.FC = () => {
	...
	const [applyingSection, setApplyingSection] = useState<CompareSectionKey | null>(null);
	const [ownProfile, setOwnProfile] = useState<ResumeProfile | null>(null);
	const [otherProfile, setOtherProfile] = useState<NormalisedCompareProfile | null>(null);
	const [evaluations, setEvaluations] = useState<Map<CompareSectionKey, SectionEvaluation>>(new Map());
	...
	const handleScrape = async () => {
		...
		if (!LINKEDIN_URL_PATTERN.test(trimmed)) {
			setUrlError("Please enter a valid LinkedIn profile URL (https://www.linkedin.com/in/...)");
			return;
		}
