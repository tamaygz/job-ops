...
compareRouter.post(
	...
	asyncRoute(async (req, res) => {
		...
		if (!LINKEDIN_URL_PATTERN.test(url)) {
			fail(res, badRequest("URL must be a LinkedIn profile URL (https://www.linkedin.com/in/<slug>)"));
			return;
		}
...
compareRouter.post(
	...
	asyncRoute(async (req, res) => {
		...
		if (!otherProfile) {
			fail(res, notFound("No cached profile found for this URL. Please scrape first."));
			return;
		}
		...
		if (input.jobId) {
			try {
				...
				logger.warn("Could not load job for compare evaluation", { jobId: input.jobId });
			}
		}
		...
		try {
			for await (const evaluation of evaluateSections(ownProfile, otherProfile, jobDescription)) {
...
compareRouter.post(
	...
	asyncRoute(async (req, res) => {
		...
		if (!VALID_ACTIONS.includes(input.action as (typeof VALID_ACTIONS)[number])) {
			fail(res, badRequest(`Invalid action: ${input.action}. Must be 'copy' or 'copy_rewrite'`));
			return;
		}
		...
		if (!otherProfile) {
			fail(res, notFound("No cached profile found for this URL. The cache may have expired."));
			return;
		}
