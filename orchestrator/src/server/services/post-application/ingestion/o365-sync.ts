...
import type { O365Credentials } from "./o365-api";
import {
  buildEmailText,
  extractBodyText,
  getMessageFull,
...
function parseFromField(from: {
  ...
  const senderName = from.emailAddress.name?.trim() || null;
...
export async function runO365IngestionSync(args: {
  accountKey: string;
  maxMessages?: number;
  searchDays?: number;
}): Promise<O365SyncSummary> {
  const integration = await getPostApplicationIntegration(
    "o365",
    args.accountKey,
  );
  const parsedCredentials = parseO365Credentials(
    integration?.credentials ?? null,
  );
  if (!integration || !parsedCredentials) {
    throw new Error(`O365 account '${args.accountKey}' is not connected.`);
  }
  ...
  try {
    ...
    await runWithConcurrency(messageIds, concurrency, async (message) => {
      ...
      try {
        ...
        const { fromAddress, fromDomain, senderName } = parseFromField(
          metadata.from,
        );
        ...
        if (existingMessage) {
          const { message: savedMessage, autoLinkTransitioned } =
            await upsertPostApplicationMessage({
              ...
              externalThreadId: metadata.conversationId,
              fromAddress,
              fromDomain,
              senderName,
        ...
        const emailText = buildEmailText({
          from: fromString,
          subject: metadata.subject,
          date: metadata.receivedDateTime,
          body,
        });
        ...
        errored += 1;
        logger.warn("Failed to ingest O365 message", {
          provider: "o365",
          accountKey: args.accountKey,
