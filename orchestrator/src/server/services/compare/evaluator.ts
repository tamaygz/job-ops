...
const VALID_VERDICTS = new Set<SectionVerdict>([
  "stronger",
  "weaker",
  "comparable",
]);
  ...
  schema: {
    ...
    properties: {
      ...
      rationale: {
        type: "string",
        description:
          "Brief plain-text explanation (60 words max) of why this verdict was chosen",
      },
    },
    ...
    return (
      items
        .map((item: unknown) => {
          ...
          return [i.position, i.company, i.date, i.summary]
            .filter(Boolean)
            .join(" | ");
        })
        .join("\n") || "(empty)"
    ...
    try {
      ...
      if (ownContent === "(empty)" && otherContent === "(empty)") {
        yield {
          section,
          verdict: "comparable",
          rationale: "Both profiles have no content in this section.",
        };
        continue;
      }
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a professional profile comparison assistant. Return structured JSON.",
            },
