...
async function fetchViaCamoufox(url: string): Promise<string> {
  const { stdout } = await execFileAsync("node", [CAMOUFOX_SCRIPT_PATH, url], {
...
async function fetchViaHttp(url: string): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) {
      throw upstreamError(`LinkedIn returned HTTP ${response.status}`, {
...
export async function scrapeLinkedInProfile(url: string): Promise<string> {
  ...
  try {
    ...
    if (error instanceof AppError) throw error;
    throw upstreamError("Failed to fetch LinkedIn profile", {
      url,
