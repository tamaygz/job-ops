...
import type { CompareSectionKey, NormalisedCompareProfile } from "@shared/types";
...
function sectionToText(
	...
	if (section === "basics") {
		...
		return [b.name, b.headline, b.location, b.summary].filter(Boolean).join("\n");
	}
...
async function rewriteWithLlm(content: string, section: CompareSectionKey): Promise<string> {
...
function buildSectionPatchOperations(
	...
	if (section === "basics") {
		ops.push({ op: "replace", path: "/basics/headline", value: otherProfile.basics.headline });
		if (otherProfile.basics.summary) {
			ops.push({ op: "replace", path: "/summary/content", value: rewrittenContent ?? otherProfile.basics.summary });
		}
		return ops;
	}
	...
	if (section === "experience") {
		...
		ops.push({ op: "replace", path: "/sections/experience/items", value: items });
		return ops;
	}
	...
	if (section === "education") {
		...
		ops.push({ op: "replace", path: "/sections/education/items", value: items });
		return ops;
	}
...
export async function applySection(
	...
	const operations = buildSectionPatchOperations(section, otherProfile, rewrittenContent);
