...
function extractJsonLd(html: string): Record<string, unknown> | null {
	...
	while ((match = pattern.exec(html)) !== null) {
		try {
			...
			if (parsed && typeof parsed === "object" && parsed["@type"] === "Person") {
...
function extractTextBetween(
	...
	if (!endMatch) return stripHtml(rest.slice(0, 500));
	return stripHtml(rest.slice(0, endMatch.index));
}

function extractName(html: string, jsonLd: Record<string, unknown> | null): string {
	if (jsonLd?.name && typeof jsonLd.name === "string") return safeString(jsonLd.name);
	const match = /<h1[^>]*>([\s\S]*?)<\/h1>/i.exec(html);
	return match ? safeString(match[1]) : "";
}

function extractHeadline(html: string, jsonLd: Record<string, unknown> | null): string {
	if (jsonLd?.jobTitle && typeof jsonLd.jobTitle === "string") return safeString(jsonLd.jobTitle);
	const match = /<div[^>]*class="[^"]*top-card-layout__headline[^"]*"[^>]*>([\s\S]*?)<\/div>/i.exec(html);
	return match ? safeString(match[1]) : "";
}

function extractLocation(html: string, jsonLd: Record<string, unknown> | null): string {
	...
	const match = /<div[^>]*class="[^"]*top-card-layout__(?:first-)?subline[^"]*"[^>]*>([\s\S]*?)<\/div>/i.exec(html);
	return match ? safeString(match[1]) : "";
}

function extractSummary(html: string, jsonLd: Record<string, unknown> | null): string {
	if (jsonLd?.description && typeof jsonLd.description === "string") {
		return safeString(jsonLd.description, MAX_SUMMARY_LENGTH);
	}
	const match = /<section[^>]*class="[^"]*summary[^"]*"[^>]*>([\s\S]*?)<\/section>/i.exec(html);
...
function extractExperience(html: string): CompareExperienceItem[] {
	...
	const sectionMatch = /<section[^>]*(?:id="experience|class="[^"]*experience)[^>]*>([\s\S]*?)<\/section>/i.exec(html);
	if (!sectionMatch) return items;
	const section = sectionMatch[1];

	const itemPattern = /<li[^>]*class="[^"]*experience-item[^"]*"[^>]*>([\s\S]*?)<\/li>/gi;
	...
	while ((itemMatch = itemPattern.exec(section)) !== null) {
		...
		const company = safeString(
			...
			(/<p[^>]*class="[^"]*subtitle[^"]*"[^>]*>([\s\S]*?)<\/p>/i.exec(block) ?? [])[1] ?? "",
		);
		const period = safeString(
			(/<span[^>]*class="[^"]*date-range[^"]*"[^>]*>([\s\S]*?)<\/span>/i.exec(block) ?? [])[1] ?? "",
		);
		const description = safeString(
			(/<p[^>]*class="[^"]*(?:description|show-more)[^"]*"[^>]*>([\s\S]*?)<\/p>/i.exec(block) ?? [])[1] ?? "",
...
function extractEducation(html: string): CompareEducationItem[] {
	...
	const sectionMatch = /<section[^>]*(?:id="education|class="[^"]*education)[^>]*>([\s\S]*?)<\/section>/i.exec(html);
	if (!sectionMatch) return items;
	const section = sectionMatch[1];

	const itemPattern = /<li[^>]*class="[^"]*education[^"]*"[^>]*>([\s\S]*?)<\/li>/gi;
	let itemMatch: RegExpExecArray | null;
	while ((itemMatch = itemPattern.exec(section)) !== null) {
		const block = itemMatch[1];
		...
		const degreeRaw = safeString(
			(/<h4[^>]*>([\s\S]*?)<\/h4>/i.exec(block) ?? [])[1] ??
			(/<span[^>]*class="[^"]*degree[^"]*"[^>]*>([\s\S]*?)<\/span>/i.exec(block) ?? [])[1] ?? "",
		);
...
function extractSkills(html: string): CompareSkillItem[] {
	...
	const sectionMatch = /<section[^>]*(?:id="skills|class="[^"]*skills)[^>]*>([\s\S]*?)<\/section>/i.exec(html);
	if (!sectionMatch) return items;
	const section = sectionMatch[1];

	const skillPattern = /<span[^>]*class="[^"]*skill-[^"]*"[^>]*>([\s\S]*?)<\/span>/gi;
	let skillMatch: RegExpExecArray | null;
	while ((skillMatch = skillPattern.exec(section)) !== null) {
...
export function normaliseLinkedInHtml(
	...
	if (!name) {
		logger.warn("LinkedIn normalisation produced no name — likely login-wall or empty page", {
			sourceUrl,
		});
